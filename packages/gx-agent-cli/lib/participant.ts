  ...
  .action(async (cmd) => {
    try {
      ...
      const sds = vcs.filter(
        (vc) => (vc.verifiableCredential.type!.includes('LegalPerson') || vc.verifiableCredential.type!.includes('ParticipantCredential')) && (!did || vc.verifiableCredential.issuer === did)
      )
      printTable(
        sds.map((sd) => {
          return {
